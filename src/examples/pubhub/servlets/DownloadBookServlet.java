...
public class DownloadBookServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// In order to download the PDF to the client, we need to add a data
		// stream to the response.
		// First we set the content type to PDF, so the browser knows how to
		// interpret the data it's receiving
		response.setContentType("application/pdf");

		// Then we set the filename to the book's title, so it's not a random
